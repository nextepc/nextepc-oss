...
void sepp_nnrf_handle_nf_register(
...
void sepp_nnrf_handle_nf_status_subscribe(
...
#define VALIDITY_MINIMUM (10LL * OGS_USEC_PER_SEC) /* 10 seconds */
        ...
        if (ogs_sbi_time_from_string(
            ...
            subscription->t_validity = ogs_timer_add(ogs_app()->timer_mgr,
                sepp_timer_subscription_validity, subscription);
...
bool sepp_nnrf_handle_nf_status_notify(
        ogs_sbi_stream_t *stream, ogs_sbi_message_t *recvmsg)
{
    ...
    if (NotificationData->event ==
        ...
        if (!nf_instance) {
            ...
            OGS_FSM_TRAN(&nf_instance->sm, sepp_nf_state_registered);
        ...
        if (!handled) {
            SEPP_NF_INSTANCE_CLEAR("NRF-notify", nf_instance);
            ogs_sbi_header_free(&header);
            return false;
        }
...
void sepp_nnrf_handle_nf_discover(
        ogs_sbi_xact_t *xact, ogs_sbi_message_t *recvmsg)
{
        ...
        if (NF_INSTANCE_IS_OTHERS(nf_instance->id)) {
            ...
            if (!handled) {
                ogs_error("ogs_sbi_nnrf_handle_nf_profile() failed [%s]",
                        nf_instance->id);
                SEPP_NF_INSTANCE_CLEAR("NRF-discover", nf_instance);
                continue;
            }
            ...
            sess = (sepp_sess_t *)sbi_object;
            ogs_assert(sess);
            sepp_sess_select_nf(sess, nf_instance->nf_type);
    ...
    if (!nf_instance) {
        ...
        sepp_sbi_send(nf_instance, xact);
    }
}
