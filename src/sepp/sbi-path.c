...
static int server_cb(ogs_sbi_request_t *request, void *data)
{
    ...
    e = sepp_event_new(SEPP_EVT_SBI_SERVER);
    ogs_assert(e);
...
static int client_cb(ogs_sbi_response_t *response, void *data)
{
    ...
    e = sepp_event_new(SEPP_EVT_SBI_CLIENT);
    ogs_assert(e);
...
int sepp_sbi_open(void)
{
    ...
    ogs_list_for_each(&ogs_sbi_self()->nf_instance_list, nf_instance) {
        ...
        ogs_sbi_nf_instance_build_default(nf_instance, sepp_self()->nf_type);

        /* Build NF service information. It will be transmitted to NRF. */
        service = ogs_sbi_nf_service_build_default(nf_instance,
                (char*)OGS_SBI_SERVICE_NAME_NSEPP_MANAGEMENT);
        ogs_assert(service);
...
void sepp_nnrf_nfm_send_nf_register(ogs_sbi_nf_instance_t *nf_instance)
{
    ...
    request = sepp_nnrf_nfm_build_register(nf_instance);
    if (!request) {
        ogs_error("sepp_nnrf_nfm_send_nf_register() failed");
        return;
    }
        ...
        ogs_sbi_request_t *(*build)(sepp_sess_t *sess, void *data))
{
    ...
    xact = ogs_sbi_xact_add(target_nf_type, &sess->sbi,
            ...
            sepp_timer_sbi_client_wait_expire);
    if (!xact) {
        ogs_error("sepp_sbi_discover_and_send() failed");
        ogs_sbi_server_send_error(stream,
                OGS_SBI_HTTP_STATUS_GATEWAY_TIMEOUT, NULL,
                "Cannot discover", sepp_ue->supi);
        return;
    }
    ...
    if (ogs_sbi_discover_and_send(xact,
            (ogs_fsm_handler_t)sepp_nf_state_registered, client_cb) != true) {
        ogs_error("sepp_sbi_discover_and_send() failed");
        ogs_sbi_server_send_error(stream,
                OGS_SBI_HTTP_STATUS_GATEWAY_TIMEOUT, NULL,
                "Cannot discover", sepp_ue->supi);
