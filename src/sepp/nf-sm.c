...
void sepp_nf_fsm_init(ogs_sbi_nf_instance_t *nf_instance)
{
    sepp_event_t e;
    ...
    ogs_fsm_create(&nf_instance->sm,
            sepp_nf_state_initial, sepp_nf_state_final);
    ogs_fsm_init(&nf_instance->sm, &e);
}

void sepp_nf_fsm_fini(ogs_sbi_nf_instance_t *nf_instance)
{
    sepp_event_t e;
...
void sepp_nf_state_initial(ogs_fsm_t *s, sepp_event_t *e)
{
    ...
    nf_instance->t_no_heartbeat->cb = sepp_timer_nf_instance_no_heartbeat;
    ogs_assert(nf_instance->t_validity);
    nf_instance->t_validity->cb = sepp_timer_nf_instance_validity;

    if (NF_INSTANCE_IS_SELF(nf_instance->id)) {
        OGS_FSM_TRAN(s, &sepp_nf_state_will_register);
    } else {
        OGS_FSM_TRAN(s, &sepp_nf_state_registered);
    }
}

void sepp_nf_state_final(ogs_fsm_t *s, sepp_event_t *e)
{
    ogs_assert(s);
    ogs_assert(e);
...
void sepp_nf_state_will_register(ogs_fsm_t *s, sepp_event_t *e)
{
    ogs_sbi_nf_instance_t *nf_instance = NULL;
    ...
    case OGS_FSM_ENTRY_SIG:
        ...
        sepp_nnrf_nfm_send_nf_register(nf_instance);
        break;
        ...
        CASE(OGS_SBI_SERVICE_NAME_NNRF_NFM)
            ...
            CASE(OGS_SBI_RESOURCE_NAME_NF_INSTANCES)
                ...
                if (message->res_status == OGS_SBI_HTTP_STATUS_OK ||
                    ...
                    sepp_nnrf_handle_nf_register(nf_instance, message);
                    OGS_FSM_TRAN(s, &sepp_nf_state_registered);
                } else {
                    ogs_error("[%s] HTTP Response Status Code [%d]",
                            nf_instance->id, message->res_status);
                    OGS_FSM_TRAN(s, &sepp_nf_state_exception);
                }
                break;
    ...
    default:
        ogs_error("[%s] Unknown event %s",
                nf_instance->id, sepp_event_get_name(e));
        break;
    }
}

void sepp_nf_state_registered(ogs_fsm_t *s, sepp_event_t *e)
{
    ogs_sbi_nf_instance_t *nf_instance = NULL;
    ogs_sbi_client_t *client = NULL;
    ogs_sbi_message_t *message = NULL;
    ...
    case SEPP_EVT_SBI_TIMER:
        ...
        case SEPP_TIMER_NF_INSTANCE_VALIDITY:
            if (NF_INSTANCE_IS_OTHERS(nf_instance->id)) {
                ...
                OGS_FSM_TRAN(s, &sepp_nf_state_de_registered);
            }
            break;
...
void sepp_nf_state_de_registered(ogs_fsm_t *s, sepp_event_t *e)
{
    ogs_sbi_nf_instance_t *nf_instance = NULL;
    ogs_assert(s);
    ogs_assert(e);
...
void sepp_nf_state_exception(ogs_fsm_t *s, sepp_event_t *e)
{
    ogs_sbi_nf_instance_t *nf_instance = NULL;
    ogs_sbi_client_t *client = NULL;
    ogs_sbi_message_t *message = NULL;
