...
{
    OpenAPI_api_signature_t *api_signature_local_var = OpenAPI_malloc(sizeof(OpenAPI_api_signature_t));
    if (!api_signature_local_var) {
        return NULL;
    }
...
OpenAPI_api_signature_t *OpenAPI_api_signature_parseFromJSON(cJSON *api_signatureJSON)
{
    ...
    cJSON *callback_type = cJSON_GetObjectItemCaseSensitive(api_signatureJSON, "callbackType");
    if (!callback_type) {
