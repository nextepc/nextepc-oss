...
{
    OpenAPI_ipx_provider_sec_info_t *ipx_provider_sec_info_local_var = OpenAPI_malloc(sizeof(OpenAPI_ipx_provider_sec_info_t));
    if (!ipx_provider_sec_info_local_var) {
        return NULL;
    }
...
cJSON *OpenAPI_ipx_provider_sec_info_convertToJSON(OpenAPI_ipx_provider_sec_info_t *ipx_provider_sec_info)
{
    ...
    if (ipx_provider_sec_info->raw_public_key_list) {
        cJSON *raw_public_key_list = cJSON_AddArrayToObject(item, "rawPublicKeyList");
        if (raw_public_key_list == NULL) {
            ogs_error("OpenAPI_ipx_provider_sec_info_convertToJSON() failed [raw_public_key_list]");
            goto end;
        }

        OpenAPI_lnode_t *raw_public_key_list_node;
        OpenAPI_list_for_each(ipx_provider_sec_info->raw_public_key_list, raw_public_key_list_node)  {
            if (cJSON_AddStringToObject(raw_public_key_list, "", (char*)raw_public_key_list_node->data) == NULL) {
                ogs_error("OpenAPI_ipx_provider_sec_info_convertToJSON() failed [raw_public_key_list]");
                goto end;
            }
        }
    ...
    if (ipx_provider_sec_info->certificate_list) {
        cJSON *certificate_list = cJSON_AddArrayToObject(item, "certificateList");
        if (certificate_list == NULL) {
            ogs_error("OpenAPI_ipx_provider_sec_info_convertToJSON() failed [certificate_list]");
            goto end;
        }

        OpenAPI_lnode_t *certificate_list_node;
        OpenAPI_list_for_each(ipx_provider_sec_info->certificate_list, certificate_list_node)  {
            if (cJSON_AddStringToObject(certificate_list, "", (char*)certificate_list_node->data) == NULL) {
                ogs_error("OpenAPI_ipx_provider_sec_info_convertToJSON() failed [certificate_list]");
                goto end;
            }
        }
...
OpenAPI_ipx_provider_sec_info_t *OpenAPI_ipx_provider_sec_info_parseFromJSON(cJSON *ipx_provider_sec_infoJSON)
{
    ...
    cJSON *ipx_provider_id = cJSON_GetObjectItemCaseSensitive(ipx_provider_sec_infoJSON, "ipxProviderId");
    if (!ipx_provider_id) {
    ...
    cJSON *raw_public_key_list = cJSON_GetObjectItemCaseSensitive(ipx_provider_sec_infoJSON, "rawPublicKeyList");

    OpenAPI_list_t *raw_public_key_listList;
    if (raw_public_key_list) {
        cJSON *raw_public_key_list_local;
    ...
    cJSON *certificate_list = cJSON_GetObjectItemCaseSensitive(ipx_provider_sec_infoJSON, "certificateList");

    OpenAPI_list_t *certificate_listList;
    if (certificate_list) {
        cJSON *certificate_list_local;
