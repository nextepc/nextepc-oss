...
{
    OpenAPI_sec_param_exch_rsp_data_t *sec_param_exch_rsp_data_local_var = OpenAPI_malloc(sizeof(OpenAPI_sec_param_exch_rsp_data_t));
    if (!sec_param_exch_rsp_data_local_var) {
        return NULL;
    }
...
cJSON *OpenAPI_sec_param_exch_rsp_data_convertToJSON(OpenAPI_sec_param_exch_rsp_data_t *sec_param_exch_rsp_data)
{
    ...
    if (sec_param_exch_rsp_data->ipx_provider_sec_info_list) {
        ...
        if (sec_param_exch_rsp_data->ipx_provider_sec_info_list) {
            OpenAPI_list_for_each(sec_param_exch_rsp_data->ipx_provider_sec_info_list, ipx_provider_sec_info_list_node) {
...
OpenAPI_sec_param_exch_rsp_data_t *OpenAPI_sec_param_exch_rsp_data_parseFromJSON(cJSON *sec_param_exch_rsp_dataJSON)
{
    ...
    cJSON *n32f_context_id = cJSON_GetObjectItemCaseSensitive(sec_param_exch_rsp_dataJSON, "n32fContextId");
    if (!n32f_context_id) {
    ...
    cJSON *selected_jwe_cipher_suite = cJSON_GetObjectItemCaseSensitive(sec_param_exch_rsp_dataJSON, "selectedJweCipherSuite");

    if (selected_jwe_cipher_suite) {
        if (!cJSON_IsString(selected_jwe_cipher_suite)) {
            ogs_error("OpenAPI_sec_param_exch_rsp_data_parseFromJSON() failed [selected_jwe_cipher_suite]");
            goto end;
        }
    }

    cJSON *selected_jws_cipher_suite = cJSON_GetObjectItemCaseSensitive(sec_param_exch_rsp_dataJSON, "selectedJwsCipherSuite");

    if (selected_jws_cipher_suite) {
        if (!cJSON_IsString(selected_jws_cipher_suite)) {
            ogs_error("OpenAPI_sec_param_exch_rsp_data_parseFromJSON() failed [selected_jws_cipher_suite]");
            goto end;
        }
    }

    cJSON *sel_protection_policy_info = cJSON_GetObjectItemCaseSensitive(sec_param_exch_rsp_dataJSON, "selProtectionPolicyInfo");
    ...
    cJSON *ipx_provider_sec_info_list = cJSON_GetObjectItemCaseSensitive(sec_param_exch_rsp_dataJSON, "ipxProviderSecInfoList");
    ...
    cJSON *sender = cJSON_GetObjectItemCaseSensitive(sec_param_exch_rsp_dataJSON, "sender");
    ...
    sec_param_exch_rsp_data_local_var = OpenAPI_sec_param_exch_rsp_data_create (
        ...
        selected_jwe_cipher_suite ? ogs_strdup(selected_jwe_cipher_suite->valuestring) : NULL,
        selected_jws_cipher_suite ? ogs_strdup(selected_jws_cipher_suite->valuestring) : NULL,
