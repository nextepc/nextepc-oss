    ...
    )
{
    ...
    if (!n32f_error_detail_local_var) {
        return NULL;
    }
...
OpenAPI_n32f_error_detail_t *OpenAPI_n32f_error_detail_parseFromJSON(cJSON *n32f_error_detailJSON)
{
    ...
    cJSON *attribute = cJSON_GetObjectItemCaseSensitive(n32f_error_detailJSON, "attribute");
    if (!attribute) {
    ...
    cJSON *msg_reconstruct_fail_reason = cJSON_GetObjectItemCaseSensitive(n32f_error_detailJSON, "msgReconstructFailReason");
